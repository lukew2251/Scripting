...
#write the header
Write-Output ("Name,Address,City") | Out-File "employee_data.csv" -Encoding ascii
...
if (-Not (Test-Path .\archives_Willis))
{
    mkdir .\archives_Willis | Out-Null
}
...
$zipfile = @{
    ...
    DestinationPath = ".\archives_Willis\empdata01"
}
...
if (-Not (Test-Path .\extracted_files_Willis))
{
    mkdir .\extracted_files_Willis | Out-Null
}

#unzip the csv file into the the extracted_file subdirectory
Expand-Archive -Path .\archives_Willis\empdata01.zip -DestinationPath .\extracted_files_Willis -Force
